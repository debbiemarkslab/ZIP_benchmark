...
if __name__ == "__main__":
    ...
    print(f"Now saving file of singleton gnomAD variants to {args.gnomad_output_folder}/gnomad_filtered_singleton_indels.csv")
    ...
    for df in tqdm(pd.read_csv(
        ...
        dfs.append(df.loc[
            (df["FILTER"] == "PASS") &
            (df['inframe_delins'] | df['inframe_ins'] | df['inframe_del'] | df['inframe_dup']) &
            (df['AC'] == 1) &
            (df['AC_raw'] == 1)
        ])
    df = pd.concat(dfs)
    all_singleton_indels_df = df
    df.to_csv(f"{args.gnomad_output_folder}/gnomad_filtered_singleton_indels.csv", index=False)

    print(f"Singleton gnomad variants saved to {args.gnomad_output_folder}/gnomad_filtered_singleton_indels.csv. Now saving deduped and aa_filtered files to {args.gnomad_output_folder}/gnomad_filtered_singleton_indels_dedup.csv and {args.gnomad_output_folder}/gnomad_filtered_singleton_indels_dedup_lt3aa.csv")
    ...
    uniparc_info = {} 
    futures = {}
    for uniparc_id in tqdm(all_singleton_indels_df["UNIPARC"]):
    ...
    chosen_uniparc_df = all_singleton_indels_df.drop_duplicates(subset=["UNIPARC", "protein_variant"]).groupby(["#CHROM", "POS", "REF", "ALT"]).apply(choose_best_uniparc_group)
    ...
    chosen_uniparc_df.to_csv(f"{args.gnomad_output_folder}/gnomad_filtered_singleton_indels_dedup.csv",index=False)
    chosen_uniparc_df[chosen_uniparc_df["mutation_depth"] <= 3].to_csv(f"{args.gnomad_output_folder}/gnomad_filtered_singleton_indels_dedup_lt3aa.csv", index=False)
    print(f"Deduplication complete. Total time taken is {time.time() - start_time}")

    print("Splitting data by uniparc id and choosing random 10%")
    uniparc_ids = chosen_uniparc_df.loc[chosen_uniparc_df["mutation_depth"] <= 3, "UNIPARC"].unique()
    state = RandomState(42)
    ordering = state.permutation(len(uniparc_ids))
    batches = np.array_split(ordering, 10)
    batch = batches[0]
    batch_uniparc_ids = uniparc_ids[batch]
    chosen_uniparc_df[chosen_uniparc_df["UNIPARC"].isin(batch_uniparc_ids)].to_csv(f"{args.gnomad_output_folder}/gnomad_filtered_singleton_indels_dedup_lt3aa_random10.csv", index=False)

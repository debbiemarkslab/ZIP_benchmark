        ...
        if labels is not None:
            ...
            if self.retrieval_aggregation_mode is not None:
                ...
                if self.retrieval_aggregation_mode=="aggregate_indel":
                    ...
                    MSA_log_prior, MSA_start, MSA_end = msa_utils.update_retrieved_MSA_log_prior_indel(self, self.MSA_log_prior, self.MSA_start, self.MSA_end, mutated_sequence[0], clustal_suffix=self.config.clustal_suffix)  
                ...
                for seq_index in range(batch_size):
                    ...
                    # with open("debug_values.pkl", "ab+") as file:
                    #     pickle.dump({"slice_prior":slice_prior.detach().cpu(),"fused_shift_log_probas":fused_shift_log_probas.detach().cpu(),
                    #                  "MSA_log_prior":MSA_log_prior.detach().cpu(),"MSA_start":MSA_start,"MSA_end":MSA_end},file)
                    if flip[seq_index]:

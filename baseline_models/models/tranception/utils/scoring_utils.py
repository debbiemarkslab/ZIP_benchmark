...
def get_tranception_scores_mutated_sequences(model, mutated_sequence_df, batch_size_inference, score_var_name, target_seq, num_workers=10, reverse=False, indel_mode=False, return_log_softmax=False):
    """
    ...
    if return_log_softmax:
        log_softmaxes = {'mutated_sequence':[],'log_softmaxes':[]}
    with torch.no_grad():
        ...
        for encoded_batch in tqdm.tqdm(ds_loader):
            ...
            if (hasattr(model.config,"retrieval_aggregation_mode")) and (model.config.retrieval_aggregation_mode is not None):
                ...
                if return_log_softmax:
                    log_softmaxes['mutated_sequence'] += list(mutated_sequence)
                    log_softmaxes['log_softmaxes'] += [fused_shift_log_probas[i][encoded_batch["attention_mask"][i, 1:].float() == 1].cpu().numpy() for i in range(fused_shift_log_probas.shape[0])]
                ...
                if return_log_softmax:
                    log_softmaxes["mutated_sequence"] += list(mutated_sequence)
                    log_softmaxes['log_softmaxes'] += [shift_logits[i][encoded_batch["attention_mask"][i, 1:].float() == 1].cpu().numpy() for i in range(shift_logits.shape[0])]
    ...
    if return_log_softmax:
        log_softmaxes = pd.DataFrame(log_softmaxes)
    if model.config.scoring_window=="sliding":
        ...
        if return_log_softmax:
            combined_log_softmaxes = {'mutated_sequence':[],'log_softmaxes':[]} 
            groups = log_softmaxes.groupby('mutated_sequence') 
            for name, group in groups:
                combined_log_softmaxes['mutated_sequence'].append(name)
                combined_log_softmaxes['log_softmaxes'].append(np.concatenate(group["log_softmaxes"].tolist()))
            log_softmaxes = pd.DataFrame(combined_log_softmaxes)
    ...
    if target_seq is not None:
        ...
        if return_log_softmax:
            return delta_scores[['mutated_sequence',score_var_name]], log_softmaxes
        else:
            return delta_scores[['mutated_sequence',score_var_name]]
    else:
        scores[score_var_name] = scores['score']
        if return_log_softmax:
            return scores[['mutated_sequence',score_var_name]], log_softmaxes
        else:
